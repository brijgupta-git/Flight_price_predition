...
def predict():
    if request.method == "POST":
        ...
        # Destination
        # Banglore = 0 (not in column)
        destination = Source = request.form["Destination"]
        if (Source == 'Cochin'):
            d_Cochin = 1
            d_Delhi = 0
            d_New_Delhi = 0
    ...
    #    'Destination_Kolkata', 'Destination_New Delhi']
        ...
        return render_template('s.html', prediction_text=f"Estimated flight price is Rs. {output}",
                               source=source, destination=destination, airline=airline, stops=Total_stops, departure=date_dep, arrival=date_arr
                               )
